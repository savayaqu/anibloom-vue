<template>
  <section class="p-2">
    <div style="display: flex; flex-direction: row; justify-content: center">
      <a v-for="category in categories" :key="category.id" class="category2" @click="loadCategoryProducts(category.id, category.name)" style="font-size: 32px; text-transform: uppercase; text-decoration: none; margin-right: 10px; font-weight: 600; padding: 10px;">{{ category.name }}</a>
    </div>
...
const loadCategoryProducts = async (categoryId, categoryName) => {
  try {
    ...
    console.error(`Error while loading products for category "${categoryName}":`, error);
  }
};
